...
class NANNRepository(PreprocessingRepository):
    def __init__(self):
        super().__init__()
    ...
    def __init_standard_scaler_stock(self, stock_name):
        path = f"data/scaled/nann/{stock_name}"
    ...
    def __workflow_analyze_stock(self, stock_name):
        ...
        self.x_train[stock_name], self.y_train[stock_name] = self.create_nann_features(self.x_train[stock_name], self.y_train[stock_name], window_size=10)
        self.x_test[stock_name], self.y_test[stock_name] = self.create_nann_features(self.x_test[stock_name], self.y_test[stock_name], window_size=10)
    ...
    def create_nann_features(self, X, y, window_size=30):
        xs = []
        ys = []
    ...
    def __save_metadata(self, stock_name, mse, mae, result):
        ...
        metadata_path = f"{path}/nann-metadata.json"
        os.makedirs(path, exist_ok=True)
    ...
    def __training_model(self, stock_name):
        path = f"data/models/{stock_name}"
        model_path = f"{path}/nann.keras"

        if os.path.exists(model_path):
            self.model[stock_name] = load_model(model_path)
        else: 
            self.model[stock_name] = Sequential([
                Dense(64, activation="relu", input_shape=[self.x_train[stock_name].shape[1],]),
                Dropout(0.2),
            ...
            os.makedirs(path, exist_ok=True)
            self.model[stock_name].save(f"{path}/nann.keras")
